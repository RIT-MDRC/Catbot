...
from ...utils.deviceMock import FakeOutputDevice


def create_output_device_component(component_name: str, dict: dict = dict()):
    """
    Create a new output device component.

    returns: (device_action, register_device, get_device, get_registered_devices, get_registered_device_names, global_store)
    """
    ...
    def device_action(func: callable) -> callable:
        ...
        def wrapper(*args, **kwargs):
            if len(args) < 1:
                ...
                or isinstance(args[0], DigitalOutputDevice)
                or isinstance(args[0], FakeOutputDevice)
            ):
                raise ValueError(
                    "First argument must be a string or a DigitalOutputDevice"
                )
